...
class GOF_Function(Function):
    @staticmethod
    def forward(ctx, weight, gabor_filter_bank):
        ctx.save_for_backward(weight, gabor_filter_bank)

        c_out, c_in, M, k, _ = weight.size()
        weight_expand = weight.view(c_out, c_in, M, 1, k, k).repeat(1, 1, 1, M, 1, 1).transpose(2, 3)
        filter_expand = gabor_filter_bank.view(M, 1, k, k).repeat(1, M, 1, 1)
        filter_expand = filter_expand.view(1, M, M, k, k).repeat(c_in, 1, 1, 1, 1)
        filter_expand = filter_expand.view(1, c_in, M, M, k, k).repeat(c_out, 1, 1, 1, 1, 1)

        output = weight_expand * filter_expand
        output = rearrange(output, 'o i m c h w -> (o m) (i c) h w')
    ...
    def backward(ctx, grad_output):
        weight, gabor_filter_bank = ctx.saved_tensors

        M, k, _ = gabor_filter_bank.size()
        co, ci, _, _ = grad_output.size()
        c_out = int(co/M)
        c_in = int(ci/M)

        grad_expand = rearrange(grad_output, '(o m) (i c) h w -> o m i c h w', o=c_out, i=c_in)
        filter_expand = gabor_filter_bank.view(M, 1, k, k).repeat(1, M, 1, 1)
        filter_expand = filter_expand.view(1, M, M, k, k).repeat(c_in, 1, 1, 1, 1)
        filter_expand = filter_expand.view(1, c_in, M, M, k, k).repeat(c_out, 1, 1, 1, 1, 1)
        grad_weight = torch.einsum('oimchw,omichw -> oichw', filter_expand, grad_expand)
...
class GDConv(MConv):
    '''
    Gabor Depth Convolutional Operation Layer
    '''
    def __init__(self, in_channels, out_channels, kernel_size, M=4, nScale=3, stride=1,
                    padding=0, dilation=1, groups=1, bias=True, expand=False, padding_mode='zeros'):
        super(GDConv, self).__init__(in_channels, out_channels, kernel_size, M, nScale, stride,
